...
public class Courses<E, V> {
    ...
    public Edge<CourseRequisite> putRequisite(CourseNumber courseNum1, 
    	...
    	if(courseMap.get(courseNum1).getCourseVertex()==null || courseMap.get(courseNum2).getCourseVertex()==null) {throw new InvalidCourseNumberException();}
    	...
    	for(Vertex<CourseNumber> u : known) {
    		if(u.equals(courseMap.get(courseNum1).getCourseVertex())) {
  ...
  public static <V,E> void DFS(Graph<V,E> g, Vertex<V> u,
    ...
    known.add(u);                              // U have been discovered
